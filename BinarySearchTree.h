...
template < class T >
int BinarySearchTree<T>::getHeight() //Public member function to get height.
{
   //DO THIS
...
template < class T >
int BinarySearchTree<T>::getHeight(TreeNode<T>* tNode) //Traverses down each side of the tree to find greatest side.
{
	//cout << "inside getHeight" << endl;
   //DO THIS
	...
	else 
	{
		...
		if (left >= right)
		{
			return left + 1; //Return the size of the left side.
		}
		else
		{
			return right + 1; //Return the size of the right side.
		}
...
template < class T >
bool BinarySearchTree<T>::isBalanced() //Public member function for isBalanced.
{
   //DO THIS
...
template < class T >
bool BinarySearchTree<T>::isBalanced(TreeNode<T>* tNode) //Uses getHeight to calculate the height and see if the two trees are balanced or not.
{
   //DO THIS
	...
	TreeNode<T>* leftSubtree = tNode->getLeft();
	TreeNode<T>* rightSubtree = tNode->getRight();
	int leftHeight = getHeight(leftSubtree); //Get the size of left subtree.
	int rightHeight = getHeight(rightSubtree); //Get the size of the right subtree.
	
	if (leftSubtree == NULL && rightSubtree == NULL) //If the children are null then it's balanced.
	{
		return true;
	}
	else if (abs(leftHeight - rightHeight) <= 1) //If the difference between the two is less than 1 we keep checking down the tree.
	{
		return isBalanced(leftSubtree);
		return isBalanced(rightSubtree);	
	}
	else
	{
		return false;
...
template < class T >
BinarySearchTree<T>* BinarySearchTree<T>::minimize() //Public function to call minimize.
{
   T** items = toArray();
...
template < class T >
void BinarySearchTree<T>::minimize(T** items, int first, int last) //Will minimize a BST to a minimum height level.
{
   //DO THIS (recursive minimize method)
	if (first > last) //Can't have these indexes/
	{
		return;
	}
	else
	{
		int mid = first + (last - first) / 2; //Use the midpoint calculation for an array.
		T* item = items[mid];
		this->insert(item); //Insert item into the BST.
		this->minimize(items, first, mid-1); //Recurse to minimize the left side of array.
		this->minimize(items, mid+1 , last); //Recurse to minimize the right side of the array.
	}
}
...
template < class T >
BinarySearchTree<T>* BinarySearchTree<T>::minimizeComplete() //Public member function to minimize a complete BST.
{
	T** items = toArray();
	BinarySearchTree<T>* bst = new BinarySearchTree<T>(compare_items, compare_keys);
	//DO THIS
...
template < class T >
void BinarySearchTree<T>::minimizeComplete(T** items, int first, int last) //Minimizes a complete BST using an items array and first and last indexes.
{
   double TOL = 0.0001;
   ...
   if (first <= last)
   {
	...
	T* itemObtained = items[mid]; //Same as regular minimize, we get our item out of our mid and insert it into the tree.
	this->insert(itemObtained);
	this->minimizeComplete(items, first, mid - 1); //Recursive on the left side.
	this->minimizeComplete(items, mid + 1, last); //Recursive on the right side.
